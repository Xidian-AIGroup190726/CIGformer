"""
File: mymodule.py
Description: This module is based on modifications made to the original work by [Author's Name] 
from the [Name of the Source Material], which can be found at [URL].
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        attn = dots.softmax(dim=-1) # [N, num_heads, num_win, win_area, win_area]
...
class SwinModule(nn.Module):
    def __init__(self, in_channels, hidden_dimension, layers, downsample_factor, num_heads, head_dim, window_size,
        ...
        self.patch_partition = PatchMerging(in_channels=in_channels, out_channels=hidden_dimension,
                                            downscaling_factor=downsample_factor)
    ...
    def forward(self, x, y=None):
        if y is None:
            ...
            for regular_block, shifted_block in self.layers:
                x = regular_block(x, y)
                x = shifted_block(x, y)
            return x.permute(0, 3, 1, 2)    # output_size: [N, hidden_dim,  H//downscaling_factor, W//downscaling_factor]
